                ...
                <tbody>
                    @foreach ($performance as $p)
                        <tr>
                            ...
                            @elseif(($meta['period'] ?? '') === 'yearly')
                                <td class="text-right">
                                    {{ number_format((float) ($p['median_resolution_days'] ?? 0), 1) }}</td>
                                <td class="text-right">{{ number_format((float) ($p['p90_resolution_days'] ?? 0), 1) }}
                                </td>
                            @else
...
========================= --}}
    @if (
        ($meta['role'] ?? '') === 'pemilik' &&
            in_array($meta['period'] ?? '', ['monthly', 'yearly']) &&
            !empty($escalationSummary))
        <div class="card no-break">
