...
<body>
    ...
    <div class="card no-break">
        ...
        {{-- Close Rate + waktu selesai --}}
        <div class="small muted">
            Close rate: <b>{{ number_format((float) ($summary['close_rate'] ?? 0), 1) }}%</b>

            {{-- ✅ tampilkan detail waktu selesai hanya untuk PEMILIK --}}
    ...
    {{-- ✅ PEMILIK ONLY: KINERJA + REKAP ESKALASI --}}
    @if (($meta['role'] ?? '') === 'pemilik')

        {{-- Kinerja Per Pengurus --}}
        @if (!empty($performance) && count($performance))
            <div class="card no-break">
                <div class="h2" style="margin-top:0;">Kinerja Per Pengurus</div>
                <div class="small muted">Akumulasi berdasarkan relasi <b>ticket_assignees</b> dalam periode.</div>
                    ...
                    <tbody>
                        @foreach ($performance as $p)
                            <tr>
                                ...
                                <td class="text-right">{{ number_format((float) ($p['avg_resolution_days'] ?? 0), 1) }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                @forelse($tickets as $t)
                    @php
                        ...
                        $ticketCode =
                            $t->ticket_code ??
                            'sp-' .
                                substr(preg_replace('/[^0-9]/', '', $t->id), -3) .
                                \Carbon\Carbon::parse($t->created_at)->format('dmy');
                    @endphp
                    <tr>
                        ...
                        <td class="nowrap">
                            {{ $t->created_at ? \Carbon\Carbon::parse($t->created_at)->format('d/m/Y') : '-' }}</td>
                        <td class="nowrap">
