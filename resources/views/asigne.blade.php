...
$(document).ready(function() {
    ...
    let currentSort = {
        column: 'createdAt',
        order: 'desc' // default pertama kali terbaru
    };
    let currentFilters = {
        status: '',
        jenis_pengaduan: ''
    ...
    $('#TicketTable tbody tr').each(function() {
        ...
        ticketsData.push({
            ...
            createdAt: parseInt($tr.data('created-at'), 10) // pastikan integer timestamp
        });
    });
    ...
    function renderTable() {
        ...
        // Hapus ikon lama dan tambahkan ikon segitiga
    ...
    $('.filter-option').click(function(e) {
        ...
        if (filterType === 'status') {
            ...
            $('#filterJenisPengaduanDisplay').text(filterValue ? `Jenis Pengaduan: ${$(this).text()}` : 'Jenis Pengaduan');
        }
    ...
    $('.page-sort-option').click(function(e) {
        ...
        currentSort.order = sortOrder; // desc = terbaru, asc = terlama
        currentPage = 1;
        renderTable();
    });
    ...
    $('#TicketTable thead th.sorting').click(function() {
        const colText = $(this).text().trim().toLowerCase();
        if (['subject', 'user', 'status'].includes(colText)) {
            currentSort.column = colText;
            currentSort.order = (currentSort.order === 'asc') ? 'desc' : 'asc'; // toggle only for string columns
            currentPage = 1;
            renderTable();
        }
    });
    ...
    // Initial render → tampilkan terbaru dulu
